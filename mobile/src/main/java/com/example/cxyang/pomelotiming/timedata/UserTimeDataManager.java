package com.example.cxyang.pomelotiming.timedata;
...
import com.example.cxyang.pomelotiming.utils.DateTransUtils;
import com.example.cxyang.pomelotiming.utils.EventUtils;
...
public class UserTimeDataManager {
    ...
    public static UserTimeDataManager getInstance(Context context){
        if(mUseTimeDataManager == null){
            mUseTimeDataManager = new UserTimeDataManager(context);
        }
    ...
    public int refreshData(int dayNumber){
        ...
        //System.out.print("EventSize: "); System.out.println(mEventList.size());
        ...
        for (int i = 0 ; i < list.size() - 1 ; i += 2){
            if(list.get(i).getClassName().equals(list.get(i+1).getClassName())){
                if(list.get(i).getEventType() != 1){
                    //Log.i(UserTimeDataManager.TAG,"   EventList 出错  ： "+list.get(i).getPackageName() +"  "+ DateUtils.formatSameDayTime(list.get(i).getTimeStamp(), System.currentTimeMillis(), DateFormat.MEDIUM, DateFormat.MEDIUM).toString());
                    list.remove(i);
                    isCheckAgain = true;
                    break;
                }
                if(list.get(i+1).getEventType() != 2){
                   // Log.i(UserTimeDataManager.TAG,"   EventList 出错 ： "+list.get(i+1).getPackageName() +"  "+ DateUtils.formatSameDayTime(list.get(i+1).getTimeStamp(), System.currentTimeMillis(), DateFormat.MEDIUM, DateFormat.MEDIUM).toString());
                    list.remove(i);
                    isCheckAgain = true;
                    break;
                }
